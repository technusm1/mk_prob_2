module TaxUtil
  ...
  ## This is the new code (written by technusm1)
  ...
  def self.calc_taxes_v2(income, status = :single)
  ...
  def self.calc_taxes_v2_precompute_with_precompute_indices(income, status = :single)
    bracket = BRACKETS_NEW[status]
    percentages = BRACKETS_NEW[:percent]
    index = 0
    case status
